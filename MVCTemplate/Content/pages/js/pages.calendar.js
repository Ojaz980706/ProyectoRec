    ...
    var PagesCalendar = function(element, options) {
        ...
        */ 
        wView.prototype.render = function() {
            ...
            if(this.weekStart >= this.weekEnd)
                this.endOfWeek = moment([calendar.year, calendar.month, calendar.date]).startOf('week').add(7 + Calendar.settings.ui.week.endOfTheWeek, "days");
            else
                this.endOfWeek = moment([calendar.year, calendar.month, calendar.date]).startOf('week').add(Calendar.settings.ui.week.endOfTheWeek, "days");
        ...
        */
        ...
        MonthView.prototype.renderEvent = function (elem, arrayIndex, days) {
            ...
            var id = 'ca_' + moment(Calendar.settings.events[arrayIndex].start).unix() + arrayIndex;
            var readonly = (Calendar.settings.events[arrayIndex].readOnly == true) ? "readonly" : "";
            var half = (Calendar.settings.events[arrayIndex].other.class);
            var eventContent = "<div class='event-container " + Calendar.settings.events[arrayIndex].class + " "+readonly+" "+half+"' "+width+" data-index=" + arrayIndex + " data-startTime=" + Calendar.settings.events[arrayIndex].start + " data-endTime=" + Calendar.settings.events[arrayIndex].end + " id=" + id + " data-id=" + id + ">"
            eventContent += "<div class='event-inner'>";
            eventContent += "<div class='event-title'>" + Calendar.settings.events[arrayIndex].title + "</div>";
            eventContent += "</div>"
            eventContent += "</div>"
